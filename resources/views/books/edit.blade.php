<x-app-layout>
    ...
    <div class="flex justify-center w-full flex-wrap">
        <div class="flex justify-around flex-wrap w-11/12">
            <form class="w-full md:w-3/4 my-5 bg-white rounded-xl p-3" action="{{route('books.update') }}" method="post">
                @method('PUT')
                @csrf
                @isset($error)
                    ...
                    <div class="w-full md:w-1/2">
                        ...
                        <div id="author_list" class="w-full">
                            ...
                            <div id="{{ 'author' . ++$authorCount }}">
                                <input class="authors rounded" name="{{ 'author' . $authorCount }}" type="text"
                                    value="{{ $author['name'] }}" onkeydown="return event.key != 'Enter';">
                                <button onclick="deleteAuthor('author{{ $authorCount }}')" type="button"
                            ...
                            @else
                            <div id="{{ 'author' . ++$authorCount }}">
                                <input onkeypress="inputAuthorFieldChanged(this)" class="authors rounded"
                                    name="{{ 'author' . $authorCount }}" type="text" value=""
                                    onkeydown="return event.key != 'Enter';">
                                <button onclick="deleteAuthor('author{{ $authorCount }}')" type="button"
                                    class="inline-flex items-center p-0.5 ml-2 text-sm text-blue-400 bg-transparent rounded-sm hover:bg-blue-200 hover:text-blue-900 dark:hover:bg-blue-300 dark:hover:text-blue-900"
                                    data-dismiss-target="#badge-dismiss-default" aria-label="Remove">
                        ...
                        <p id="create_author_label" class="text-blue-800 cursor-pointer mb-5" style="margin-top:-10px"
                            onclick="createAuthorInputField('{{++$authorCount}}')">Add Another Author</p>
                        @else
                        <p id="create_author_label" class="text-blue-800 cursor-pointer mb-5"
                            style="margin-top:-10px; display: none"
                            onclick="createAuthorInputField('{{++$authorCount}}')">Add Another Author</p>
                        @endif
                        <label>Number of Pages</label>
                        <input class="w-full md:w-5/6 rounded mb-5 p-2" name="total_pages" type="text"
                            value="{{$book['total_pages'] }}" onkeydown="return event.key != 'Enter';">
                        <br>
                        <label>Number of Pages Read</label>
                        <input class="w-full md:w-5/6 rounded mb-5 p-2" name="read_pages" type="text"
                            value="{{$book['read_pages'] }}" onkeydown="return event.key != 'Enter';">
                    </div>
                    <div class="w-full md:w-1/2">
                        ...
                        <div id="publisher_list">
                            ...
                            <div id="{{'publisher' . ++$publisherCount }}">
                                <input class="publishers rounded" name="{{ 'publisher' . $publisherCount }}" type="text"
                                    value="{{$publisher['name'] }}" onkeydown="return event.key != 'Enter';">
                                <button onclick="deletePublisher('publisher{{$publisherCount}}')" type="button"
                                    class="inline-flex items-center p-0.5 ml-2 text-sm text-blue-400 bg-transparent rounded-sm hover:bg-blue-200 hover:text-blue-900 dark:hover:bg-blue-300 dark:hover:text-blue-900"
                                    data-dismiss-target="#badge-dismiss-default" aria-label="Remove">
                                    <svg aria-hidden="true" class="w-3.5 h-3.5" aria-hidden="true" fill="currentColor"
                                        viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            ...
                            @else
                            <div id="{{'publisher' . ++$publisherCount }}">
                                <input onkeypress="inputPublisherFieldChanged(this)" class="publishers rounded"
                                    name="{{ 'publisher' . $publisherCount }}" type="text" value=""
                                    onkeydown="return event.key != 'Enter';">
                                <button onclick="deletePublisher('publisher{{$publisherCount}}')" type="button"
                                    class="inline-flex items-center p-0.5 ml-2 text-sm text-blue-400 bg-transparent rounded-sm hover:bg-blue-200 hover:text-blue-900 dark:hover:bg-blue-300 dark:hover:text-blue-900"
                                    data-dismiss-target="#badge-dismiss-default" aria-label="Remove">
                        ...
                        @if(count($book['publishers']) > 0)
                        <p id="create_publisher_label" class="text-blue-800 cursor-pointer mb-5"
                            style="margin-top:-10px" onclick="createPublisherInputField('{{++$publisherCount}}')">Add
                            Another Publisher</p>
                        @else
                        <p id="create_publisher_label" class="text-blue-800 cursor-pointer mb-5"
                            style="margin-top:-10px; display:none"
                            onclick="createPublisherInputField('{{++$publisherCount}}')">Add Another Publisher</p>
                        @endif
                        <label>Publish Date</label>
                        <input class="w-full md:w-5/6 rounded mb-5 p-2" name="publish_date" type="text"
                            value="{{$book['publish_date'] }}" onkeydown="return event.key != 'Enter';">
                    </div>
                </div>

                <label>Description</label>
                <textarea class="w-full rounded mb-5 p-2" name="description"
                    ...
                    <div id="subject_list" class="w-1/2">
                        <div style="display:none" class="inline" id="subject0">
                            ...
                            <span id="badge-dismiss-green"
                                ...
                                <span></span>
                                <button onclick="deleteSubject('subject0')" type="button"
                        ...
                        @if($index < 3) <div class="inline" id="{{'subject' . ++$subjectCount }}">
                            ...
                            <span id="badge-dismiss-green"
                                ...
                                <button onclick="deleteSubject('subject{{$subjectCount}}')" type="button"
                                    class="inline-flex items-center p-0.5 ml-2 text-sm text-green-400 bg-transparent rounded-sm hover:bg-green-200 hover:text-green-900 dark:hover:bg-green-300 dark:hover:text-green-900"
                                    data-dismiss-target="#badge-dismiss-green" aria-label="Remove">
                                    <svg aria-hidden="true" class="w-3.5 h-3.5" aria-hidden="true" fill="currentColor"
                                        viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
        ...
        <textarea class="w-full rounded mb-5 p-2" name="public_comment" type="text"
            value="{{ $book['public_comment'] }}"></textarea>

        <button type="submit" class="block bg-blue-800 hover:bg-blue-500 rounded text-white py-3 px-5 my-3">Update
            book</button>
        </form>
    </div>
    </div>
